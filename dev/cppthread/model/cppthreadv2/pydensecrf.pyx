...
cdef extern from "densecrf.h":
    cdef cppclass DenseCRF:
        ...
        void mpinference(const float *unary, const float *reference, float *out)
...
cdef class PyDenseCRF:
    ...
    def mpinference(self, float[:] unary, float[:] reference, float[:] out):
        self.dcrf.mpinference(&unary[0], &reference[0], &out[0])
