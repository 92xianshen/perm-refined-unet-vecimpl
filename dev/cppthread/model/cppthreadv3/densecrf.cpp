...
void DenseCRF::mtinference(const float *unary, const float *reference,
    ...
    for (int i = 0; i < n_iterations_; i++) {
        ...
        auto normDim = [this](const float *tbilateral_out,
        ...
        for (int i = 0; i < n_thread_; i++) {
            const float *pbilateral_out = bilateral_out + i * tN_ * n_classes_;
            const float *pspatial_out = spatial_out + i * tN_ * n_classes_;
            const float *pbilateral_norm_vals = bilateral_norm_vals + i * tN_;
            const float *pspatial_norm_vals = spatial_norm_vals + i * tN_;
            const float *punary = unary + i * tN_ * n_classes_;
            float *pQ = Q + i * tN_ * n_classes_;
